...
then you first need to initialize the git repo using ``git init``.)

Then transfer (push) the local repository to Heroku and reset the oTree database on Heroku.
...
click to edit the dynos, and turn on the "worker"
dyno.
...
.. note::
    ...
    Also, If you do not see a "worker" entry, make sure your ``Procfile``
