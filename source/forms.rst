...
Django fields example: table of radio buttons
'''''''''''''''''''''''''''''''''''''''''''''
...
First, define your ``Page`` in views.py:
...
In the template, the yes/no options must be in separate table cells,
so the ordinary ``RadioSelectHorizontal`` widget cannot be used.
So, you can skip using ``{% formfield %}`` entirely,
and use Django's `field attributes <https://docs.djangoproject.com/en/1.9/topics/forms/#rendering-fields-manually>`__:
...
(e.g. the structure of ``<div>`` and ``class`` attributes).
